package cn.stanliski.framework.dao.impl;
...
import cn.stanliski.framework.annotation.AliasAnnotation;
import cn.stanliski.framework.annotation.Operation;
import cn.stanliski.framework.annotation.OperationType;
					...
					if(o != null){
						...
						if(op != null ){
							if(op.association().equals("")){
								...
								c.createAlias(op.association(), op.association(),CriteriaSpecification.LEFT_JOIN);
								c.add(Restrictions.eq( op.association()+".id" ,  o));
							}
						}
...
/*		Long size = (Long) c.setProjection(Projections.rowCount()).uniqueResult();  
	...
	private Long getSize(BaseFilter filter)
	{
		Criteria c = getCriteria(filter);
	...
	public List findPage(Class clazz,BaseFilter filter, Page page) {
		Criteria c = getAliasCriteria(filter);
		...
		System.arraycopy(superField, 0, all, field.length, superField.length);
		for(Field f : all){
			AliasAnnotation aa = f.getAnnotation(AliasAnnotation.class);
			f.setAccessible(true);
			if(!f.getName().equals("serialVersionUID") 
					&& !f.getName().equals("curPageNo")
					&& !f.getName().equals("pageSize")
					&& !List.class.isAssignableFrom(f.getType()) ){
				if(aa == null){
					projectionList.add(Projections.property(f.getName()),f.getName()); 
				}else{
					projectionList.add(Projections.property(aa.name() + "." + aa.propertyName()),f.getName());
				}
		...
		List t = c.list().size() == 0 ? null : c.list();
		return t;
	}
...
//		c.setProjection(null);
	 ...
	 * @return
	 */
	private Criteria getAliasCriteria(BaseFilter filter) {
		Criteria c = getSession().createCriteria(entityClass);
		List<Criterion> criterionList = new ArrayList<Criterion>();
		c.setCacheable(false);
		for(Field f : entityClass.getDeclaredFields()){
			...
			if(jc != null){
				c.createAlias(f.getName(), f.getName(),CriteriaSpecification.LEFT_JOIN);
			}
		}
		if(filter != null ){
			if(filter.isCache() == true){
